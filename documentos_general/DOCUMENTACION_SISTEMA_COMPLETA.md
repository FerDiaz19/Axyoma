...
- Se crea registro en `SuscripcionEmpresa` con `estado=True`
...
```
Empresa
└── SuscripcionEmpresa (estado=true)
└── Planta(s)
    └── AdminPlanta → Usuario
    └── Departamento(s)
        └── Puesto(s)
...
(1, 1, 2, '2025-01-01 00:00:00', '2025-02-01 00:00:00', true, '2025-01-01 00:00:00');
...
```sql
-- PROBLEMA: La tabla tiene constraint que no permite boolean True
-- SOLUCIÓN: Verificar y corregir constraint en la base de datos

-- Ver constraint actual:
SELECT conname, pg_get_constraintdef(oid) 
FROM pg_constraint 
WHERE conrelid = 'subscriptions_suscripcionempresa'::regclass;

-- Si el constraint está mal, eliminarlo y recrear:
ALTER TABLE subscriptions_suscripcionempresa DROP CONSTRAINT IF EXISTS suscripciones_estado_check;

-- O cambiar el campo a usar valores correctos según el constraint
-- Verificar qué valores permite el campo estado
```
