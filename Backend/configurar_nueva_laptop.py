...
def limpiar_base_datos():
    ...
    comandos_limpieza = [
        ("python manage.py flush --noinput", "Limpiar datos existentes"),
        ("python manage.py migrate --run-syncdb", "Sincronizar estructura")
    ]
    
    for comando, descripcion in comandos_limpieza:
        if not ejecutar_comando(comando, descripcion):
            print(f"⚠️ Advertencia en: {descripcion}")
            # Continuar aunque haya errores en la limpieza
...
def configurar_base_datos():
    ...
    print("✅ Base de datos configurada desde cero")
    return True
